...
const roles = [
  ["alphonse_mock_crm_migrator", process.env.MOCK_CRM_MIGRATION_DATABASE_PASSWORD],
...
try {
  ...
  await client.query(`GRANT ${identifier("alphonse_crm_gateway")}, ${identifier("alphonse_mock_crm")}
    TO ${identifier("alphonse_mock_crm_migrator")}`);
  const exists = await client.query("SELECT 1 FROM pg_database WHERE datname=$1", [databaseName]);
  if (!exists.rows[0]) await client.query(`CREATE DATABASE ${identifier(databaseName)} OWNER ${identifier("alphonse_mock_crm_migrator")}`);
  else await client.query(`ALTER DATABASE ${identifier(databaseName)} OWNER TO ${identifier("alphonse_mock_crm_migrator")}`);
