...
export const CORRELATION_RULES = Object.freeze({
  ...
  relationship_bases: [
    "exact_propagated_logical_operation",
    "exact_typed_delivery_identity",
    "exact_typed_request_identity",
    "scoped_exact_value_token_equality",
...
function tokenDependency(observation, claimField) {
  ...
  if (candidates.length !== 1) {
    return { status: candidates.length ? "ambiguous_provenance" : "missing_provenance", dependency: null };
  }
...
function observationNode(observation, identityField) {
  ...
  return {
    node_key: `observation_${sha256Digest({
...
export function buildCorrelationProjection({
  ...
  const operationNode = {
    ...
    node_type: "logical_operation",
    claimed_identity: logicalOperationId,
    receipt_reference: null,
    identity_claim_location: null
  };
  nodes.push(operationNode);
  for (const observation of selected) {
    const identityField = CORRELATION_RULES.identity_claims[observation.observation_type];
    const node = observationNode(observation, identityField);
    if (!node) {
  ...
  for (const source of sources) {
    edges.push(makeEdge("logical_operation_contains_delivery", operationNode.node_key,
      nodeByReceipt.get(source.receipt_id).node_key, "exact_propagated_logical_operation", [
        claimLocation(source, "logical_operation_id")
      ]));
  }
    ...
    if (deliveryId && candidates.length === 1) {
      edges.push(makeEdge(relationship, nodeByReceipt.get(candidates[0].receipt_id).node_key,
        nodeByReceipt.get(observation.receipt_id).node_key, "exact_typed_delivery_identity", [
          ...
          claimLocation(candidates[0], "logical_operation_id"), claimLocation(observation, "logical_operation_id")
        ]));
      return candidates[0];
    }
  ...
  for (const request of requests) {
    requestSourceMatches.set(request.receipt_id,
  ...
  for (const request of requests) {
    ...
    const sourceTokenCandidates = sources.map((source) => ({
      source,
    ...
    if (sourceTokenCandidates.length !== 1) {
      unresolved.push(makeUnresolved("delivery_identity_equals_request_key", request.receipt_id,
        sourceTokenCandidates.length ? "ambiguous_scoped_equality" : "scoped_equality_not_established",
        [claimLocation(request, "idempotency_key_equality_token")],
        sourceTokenCandidates.map(({ source }) => nodeByReceipt.get(source.receipt_id).node_key)));
      continue;
    }
    const tokenSource = sourceTokenCandidates[0].source;
    if (requestSourceMatches.get(request.receipt_id)?.receipt_id !== tokenSource.receipt_id) {
      unresolved.push(makeUnresolved("delivery_identity_equals_request_key", request.receipt_id,
        "typed_identity_and_token_relationship_disagree", [
          ...
          claimLocation(tokenSource, "delivery_id"), claimLocation(tokenSource, "delivery_identity_equality_token")
        ], [nodeByReceipt.get(tokenSource.receipt_id).node_key]));
      continue;
    }
    edges.push(makeEdge("delivery_identity_equals_request_key",
      nodeByReceipt.get(tokenSource.receipt_id).node_key, nodeByReceipt.get(request.receipt_id).node_key,
      "scoped_exact_value_token_equality", [
        claimLocation(tokenSource, "delivery_identity_equality_token"),
        claimLocation(request, "idempotency_key_equality_token")
      ], [provenanceReference(sourceTokenCandidates[0].token.dependency),
        provenanceReference(requestToken.dependency)]));
  }

  const orderedRequests = [...requests].sort((left, right) => compareText(
    nodeByReceipt.get(left.receipt_id).node_key,
    nodeByReceipt.get(right.receipt_id).node_key
  ));
  for (let leftIndex = 0; leftIndex < orderedRequests.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < orderedRequests.length; rightIndex += 1) {
      ...
      if (sameScope && leftToken.dependency.equality_token !== rightToken.dependency.equality_token) {
        ...
        unresolved.push(makeUnresolved("request_keys_are_distinct",
          `${left.receipt_id}:${right.receipt_id}`, sameScope ? "token_values_not_distinct" : "token_scopes_not_comparable",
          [claimLocation(left, "idempotency_key_equality_token"),
            claimLocation(right, "idempotency_key_equality_token")],
  ...
  for (const effect of effects) {
    ...
    if (requestId && candidates.length === 1) {
      edges.push(makeEdge("request_reported_ledger_claim", nodeByReceipt.get(candidates[0].receipt_id).node_key,
      ...
      unresolved.push(makeUnresolved("request_reported_ledger_claim", effect.receipt_id,
  ...
  const semanticProjection = {
    ...
    manifests: {
      ...
      receipts: canonicalList(receiptManifest),
      schemas: canonicalList(schemaManifest),
      tokenization_provenance: canonicalList(tokenizationManifest)
    },
    coverage: {
      streams: coverage,
    ...
    graph: {
      ...
      nodes: canonicalList(nodes),
      edges: canonicalList(edges),
      unresolved_relationships: canonicalList(unresolved)
    },
    authority: {
      diagnosis_established: false,
      responsible_workflow_node_established: false,
      defect_established: false,
