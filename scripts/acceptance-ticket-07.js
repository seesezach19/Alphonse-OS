...
async function waitUntilHealthy() {
  ...
  throw new Error("Ticket 07 services did not become healthy.");
}
...
try {
  ...
  const launched = launchInventoryComparison(workloadGrant, "alphonse-reference-workload:ticket-06",
  ...
  await new Promise((resolve) => setTimeout(resolve, Math.max(0, Date.parse(acceptedDelegationExpiry) - Date.now() + 100)));
  ...
  assert.equal(handoffOverview.body.runs.count, 1);
  const projectedRun = handoffOverview.body.runs.items[0];
  ...
  console.log("Ticket 07 black-box acceptance passed.");
} finally {
  compose("down", "--volumes", "--remove-orphans");
}
