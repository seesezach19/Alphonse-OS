...
import { launchInventoryComparison } from "../src/reference-linux-substrate.js";

const kernelUrl = "http://127.0.0.1:43106";
const dataPlaneUrl = "http://127.0.0.1:43116";
const agentToken = "ticket-05-agent-token-0000000000000001";
const targetAgentToken = "ticket-06-target-agent-token-0000000000001";
const plainLanguageWorkflow = "When ERP and storefront inventory disagree, identify the discrepancy and correct one staging SKU with evidence and safe recovery.";
const authHeaders = { "content-type": "application/json", authorization: "Bearer local-development-bootstrap-token" };
const rehearsalMetrics = { started_at_ms: Date.now(), environment_setup_ms: 0, explicit_wait_ms: 0 };
const composeEnvironment = {
  ...process.env,
  COMPOSE_PROJECT_NAME: "alphonse-kernel-ticket-09-acceptance",
  KERNEL_PORT: "43106",
  POSTGRES_PORT: "45437",
  DATA_PLANE_PORT: "43116"
};
...
function armAdapterFault(mode) {
  const script = `fetch('http://127.0.0.1:3400/internal/test/faults/next-dispatch',{method:'POST',headers:{'content-type':'application/json',authorization:'Bearer local-adapter-test-control-token'},body:JSON.stringify({mode:${JSON.stringify(mode)}})}).then(async r=>{const body=await r.text();if(!r.ok){console.error(body);process.exit(1)}}).catch(error=>{console.error(error);process.exit(1)})`;
  const result = spawnSync("docker", ["compose", "exec", "-T", "trusted-adapter", "node", "-e", script], {
    cwd: new URL("..", import.meta.url), env: composeEnvironment, encoding: "utf8", windowsHide: true
  });
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`);
}

function armReconciliationFault() {
  const script = `fetch('http://127.0.0.1:3400/internal/test/faults/next-reconciliation',{method:'POST',headers:{'content-type':'application/json',authorization:'Bearer local-adapter-test-control-token'},body:JSON.stringify({mode:'drop_before_observation'})}).then(async r=>{const body=await r.text();if(!r.ok){console.error(body);process.exit(1)}}).catch(error=>{console.error(error);process.exit(1)})`;
  const result = spawnSync("docker", ["compose", "exec", "-T", "trusted-adapter", "node", "-e", script], {
    cwd: new URL("..", import.meta.url), env: composeEnvironment, encoding: "utf8", windowsHide: true
  });
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`);
...
async function waitUntilHealthy() {
  ...
  throw new Error("Ticket 09 services did not become healthy.");
}

async function explicitWait(milliseconds) {
  rehearsalMetrics.explicit_wait_ms += milliseconds;
    ...
    exports: [
      ...
      },
      {
        ...
        content: { effect_class: "read_only", skill_ref: "compare_inventory",
          context_requirements: { authority: ["authoritative", "representational"], max_age_seconds: 300 },
          accountability_contract_ref: "inventory_comparison_accountability" }
      },
      {
      ...
      {
        kind: "accountability_contract", export_id: "inventory_comparison_accountability", contract_version: "1.0.0",
        content: { outcome: "typed inventory discrepancy is linked to exact fresh source observations",
          evidence_requirements: ["signed source links", "typed discrepancy result"], deadline_seconds: 120,
...
try {
  const environmentSetupStartedAt = Date.now();
  compose("up", "--build", "--wait");
  rehearsalMetrics.environment_setup_ms = Date.now() - environmentSetupStartedAt;

  const initialOverview = await kernel("/kernel/v0/accountable-work/overview");
  assert.equal(initialOverview.response.status, 200);
  assert.equal(initialOverview.body.accountable_work.count, 0);
  assert.equal(initialOverview.body.deployments.count, 0);
  ...
  const proposal = await postAgent("/kernel/v0/work-intent-proposals", command("t04-proposal", "kernel.work_intent.propose", {
    ...
    objective: plainLanguageWorkflow,
  ...
  const readCapabilityExport = exportEntry("inventory_read");
  const plan = {
    ...
    { capability_export_id: readCapabilityExport.export_id,
      capability_export_digest: sha256Digest(readCapabilityExport.content),
      context_binding: { sources: ["erp", "storefront"],
        authority: ["authoritative", "representational"], max_age_seconds: 300 },
  ...
  const readCardResult = await kernel(`/kernel/v0/deployments/${deploymentId}/capabilities/inventory_read/action-card`);
  assert.equal(readCardResult.response.status, 200);
  const readCard = readCardResult.body.action_card;
  assert.equal(readCard.credential_scope, null);
  assert.deepEqual(readCard.write_target, []);
  const readApprovalInput = {
    deployment_id: deploymentId, capability_export_id: "inventory_read",
    capability_export_digest: readCard.affected_objects.capability_export_digest,
    authority_digest: readCard.affected_objects.authority_digest,
    action_card_digest: readCard.action_card_digest, expected_revision: readCard.current_revision
  };
  const readApproved = await post("/kernel/v0/capability-business-approvals", command("t07-read-approval",
    "kernel.capability.business_approve", readApprovalInput));
  assert.equal(readApproved.response.status, 201);
  const readBusinessApprovalId = readApproved.body.business_approval.business_approval_id;
  const readActivationCard = (await kernel(`/kernel/v0/deployments/${deploymentId}/capabilities/inventory_read/action-card`)).body.action_card;
  const readActivated = await post("/kernel/v0/capability-activations", command("t07-read-activation",
    "kernel.capability_activation.activate", {
      business_approval_id: readBusinessApprovalId, deployment_id: deploymentId, capability_export_id: "inventory_read",
      capability_export_digest: readActivationCard.affected_objects.capability_export_digest,
      authority_digest: readActivationCard.affected_objects.authority_digest,
      action_card_digest: readActivationCard.action_card_digest, expected_revision: readActivationCard.current_revision
    }));
  assert.equal(readActivated.response.status, 201);
  const readCapabilityActivationId = readActivated.body.capability_activation.capability_activation_id;
  ...
  const build = spawnSync("docker", ["build", "-q", "-f", "runtime/reference-workload/Dockerfile",
    "-t", "alphonse-reference-workload:ticket-08", "."],
    { cwd: new URL("..", import.meta.url), encoding: "utf8", windowsHide: true });
  assert.equal(build.status, 0, build.stderr);
  const inspect = spawnSync("docker", ["image", "inspect", "--format", "{{.Id}}", "alphonse-reference-workload:ticket-08"],
  ...
  function handoffInput(expiresAt, delegationExpiresAt = expiresAt) {
    return {
      ...
      exact_bindings: {
        ...
        deployment_id: deploymentId, capability_activation_id: readCapabilityActivationId,
        capability_export_id: readCapabilityExport.export_id,
        capability_contract_version: readCapabilityExport.contract_version,
        capability_export_digest: readApprovalInput.capability_export_digest,
        authority_digest: readApprovalInput.authority_digest
      },
  ...
  const shortExpiry = new Date(Date.now() + 3_000).toISOString();
  ...
  await explicitWait(Math.max(0, Date.parse(shortExpiry) - Date.now() + 250));
  ...
  const acceptedDelegationExpiry = new Date(Date.now() + 20_000).toISOString();
  const acceptedProposal = await postAgent("/kernel/v0/handoffs", command("t06-propose-accepted", "kernel.handoff.propose",
    handoffInput(normalExpiry, acceptedDelegationExpiry)));
  ...
  const delegationId = acceptedHandoff.body.delegation.delegation_id;
  ...
  const targetContextExpiry = new Date(Date.parse(acceptedDelegationExpiry) - 1_000).toISOString();
  const targetContextGrant = await post("/kernel/v0/context-access-grants", command("t07-target-context-grant",
    "kernel.context_access_grant.issue", { passport_id: targetPassportId, work_intent_id: activationWorkIntentId,
      delegation_id: delegationId, purpose: "Execute exact delegated inventory comparison.", subjects: ["SKU-100"],
      sources: ["erp", "storefront"], sensitivity_classes: ["internal"], max_items: 2, max_age_seconds: 300,
      expires_at: targetContextExpiry }));
  assert.equal(targetContextGrant.response.status, 201, JSON.stringify(targetContextGrant.body));
  const runtimeDelivery = await request(dataPlaneUrl, "/v0/inventory/query", {
    method: "POST", headers: { "content-type": "application/json", authorization: `Agent ${targetAgentToken}` },
    body: JSON.stringify({ grant_id: targetContextGrant.body.context_access_grant.grant_id,
      subjects: ["SKU-100"], sources: ["erp", "storefront"] })
  });
  assert.equal(runtimeDelivery.response.status, 200, JSON.stringify(runtimeDelivery.body));
  const runtimeContextReceiptId = runtimeDelivery.body.delivery.receipt_id;
  const runtimeReceipt = (await kernel(`/kernel/v0/context-receipts/${runtimeContextReceiptId}`)).body.context_receipt;
  const compareSkill = exportEntry("compare_inventory");
  const comparisonAccountability = exportEntry("inventory_comparison_accountability");
  const contextFreshnessExpiry = Math.min(...runtimeReceipt.freshness_claims
    .map((claim) => Date.parse(claim.observed_at) + 300_000));
  const executionExpiry = new Date(Math.min(Date.parse(targetContextExpiry), contextFreshnessExpiry) - 500).toISOString();
  function admissionInput(idempotencyKey) {
    return { idempotency_key: idempotencyKey, passport_id: targetPassportId,
      work_intent_id: activationWorkIntentId, delegation_id: delegationId,
      capability_activation_id: readCapabilityActivationId, package_version_id: packageVersionId,
      skill: { export_id: compareSkill.export_id, contract_version: compareSkill.contract_version,
        export_digest: sha256Digest(compareSkill.content) }, context_receipt_ids: [runtimeContextReceiptId],
      limits: { subjects: ["SKU-100"], sources: ["erp", "storefront"], max_items: 2,
        max_context_age_seconds: 300 },
      evidence_requirements: comparisonAccountability.content.evidence_requirements, expires_at: executionExpiry };
  }

  const wrongPassport = await postAgent("/kernel/v0/execution-envelopes", command("t07-wrong-passport",
    "kernel.execution_envelope.admit", admissionInput("comparison:wrong-passport")));
  assert.equal(wrongPassport.response.status, 403);
  assert.equal(wrongPassport.body.error.code, "EXECUTION_PASSPORT_MISMATCH");
  const staleInput = admissionInput("comparison:stale");
  staleInput.limits.max_context_age_seconds = 1;
  const stale = await postTarget("/kernel/v0/execution-envelopes", command("t07-stale",
    "kernel.execution_envelope.admit", staleInput));
  assert.equal(stale.response.status, 409);
  assert.equal(stale.body.error.code, "STALE_CONTEXT");
  const inactiveInput = admissionInput("comparison:inactive");
  inactiveInput.capability_activation_id = "00000000-0000-4000-8000-000000000799";
  const inactive = await postTarget("/kernel/v0/execution-envelopes", command("t07-inactive",
    "kernel.execution_envelope.admit", inactiveInput));
  assert.equal(inactive.response.status, 409);
  assert.equal(inactive.body.error.code, "CAPABILITY_INACTIVE");
  const excessInput = admissionInput("comparison:excess");
  excessInput.limits.max_items = 3;
  const excess = await postTarget("/kernel/v0/execution-envelopes", command("t07-excess",
    "kernel.execution_envelope.admit", excessInput));
  assert.equal(excess.response.status, 409);
  assert.equal(excess.body.error.code, "EXECUTION_BOUNDS_EXCEEDED");

  const exactAdmission = admissionInput("comparison:SKU-100:v1");
  const admissionCommand = command("t07-admit", "kernel.execution_envelope.admit", exactAdmission);
  const admittedRun = await postTarget("/kernel/v0/execution-envelopes", admissionCommand);
  assert.equal(admittedRun.response.status, 201, JSON.stringify(admittedRun.body));
  assert.equal(admittedRun.body.execution_envelope.external_effect_authority, false);
  assert.equal(admittedRun.body.run.execution_status, "admitted");
  assert.equal(admittedRun.body.run.accountability_status, "pending");
  assert.equal(admittedRun.body.operational_obligations.length, comparisonAccountability.content.evidence_requirements.length);
  const executionEnvelopeId = admittedRun.body.execution_envelope.envelope_id;
  const runId = admittedRun.body.run.run_id;
  const admittedReplay = await postTarget("/kernel/v0/execution-envelopes", admissionCommand);
  assert.equal(admittedReplay.response.status, 200);
  assert.equal(admittedReplay.body.execution_envelope.envelope_id, executionEnvelopeId);
  assert.equal(admittedReplay.body.run.run_id, runId);
  const domainReplay = await postTarget("/kernel/v0/execution-envelopes", command("t07-admit-domain-replay",
    "kernel.execution_envelope.admit", exactAdmission));
  assert.equal(domainReplay.response.status, 201);
  assert.equal(domainReplay.body.domain_replayed, true);
  assert.equal(domainReplay.body.run.run_id, runId);
  const replayedRun = await kernel(`/kernel/v0/runs/${runId}`);
  assert.equal(replayedRun.body.run.obligations.length, comparisonAccountability.content.evidence_requirements.length);

  const launched = launchInventoryComparison(workloadGrant, "alphonse-reference-workload:ticket-08",
    "local-workload-grant-signing-secret", { program: compareSkill.content.program,
      observations: runtimeDelivery.body.data });
  ...
  assert.deepEqual(launched.comparison, { discrepancy: 6, correction_required: true });

  const completionObservations = runtimeDelivery.body.data.map((item) => {
    const reference = runtimeReceipt.item_references.find((candidate) => candidate.source === item.source
      && candidate.subject === item.sku);
    return { source: item.source, subject: item.sku, quantity: item.quantity, observed_at: item.observed_at,
      item_hash: reference.item_hash };
  });
  const completionCommand = command("t07-complete", "kernel.run.complete_comparison",
    { run_id: runId, envelope_id: executionEnvelopeId,
      observations: completionObservations, output: launched.comparison });
  const completed = await postTarget(`/kernel/v0/runs/${runId}/complete-comparison`, completionCommand);
  assert.equal(completed.response.status, 201, JSON.stringify(completed.body));
  assert.equal(completed.body.run.execution_status, "completed");
  assert.equal(completed.body.run.accountability_status, "satisfied");
  assert.ok(completed.body.operational_obligations.every((item) => item.status === "satisfied"));
  assert.equal(completed.body.evidence_record.source_links.length, 2);
  assert.ok(completed.body.evidence_record.source_links.every((link) => link.context_receipt_id === runtimeContextReceiptId));
  const evidenceRecordId = completed.body.evidence_record.evidence_record_id;
  const unauthorizedReplay = await postAgent(`/kernel/v0/runs/${runId}/complete-comparison`, completionCommand);
  assert.equal(unauthorizedReplay.response.status, 403);
  assert.equal(unauthorizedReplay.body.error.code, "EXECUTION_PASSPORT_MISMATCH");

  const storefrontPort = spawnSync("docker", ["compose", "port", "storefront-staging", "3200"], {
    cwd: new URL("..", import.meta.url), env: composeEnvironment, encoding: "utf8", windowsHide: true
  });
  assert.ok([0, 1].includes(storefrontPort.status), storefrontPort.stderr);
  assert.equal(storefrontPort.stdout.trim(), "", "staging storefront must not publish a host port");
  assert.equal(launched.output.boundary_checks.network_default_denied, true);

  const correctionCapability = exportEntry("inventory_correction");
  const correctionAdapter = exportEntry("storefront_inventory_adapter");
  const effectExpiry = new Date(Math.min(Date.parse(executionExpiry), Date.now() + 12_000)).toISOString();
  function correctionInput(idempotencyKey = "storefront.inventory:set_quantity:SKU-100:24:v1") {
    return { effect_idempotency_key: idempotencyKey, passport_id: targetPassportId,
      work_intent_id: activationWorkIntentId, delegation_id: delegationId,
      workload_grant_id: workloadGrant.workload_grant_id,
      capability_activation_id: capabilityActivationId, package_version_id: packageVersionId,
      context_receipt_ids: [runtimeContextReceiptId],
      ...
      credential_binding: { binding_ref: "credential://storefront/inventory-writer",
        revision: "storefront-writer-rev-7", scopes: ["storefront.inventory.write"] },
      adapter: { export_id: correctionAdapter.export_id, contract_version: correctionAdapter.contract_version,
        export_digest: sha256Digest(correctionAdapter.content) },
      evidence_requirements: correctionCapability.content.evidence.required,
      recovery: correctionCapability.content.recovery, expires_at: effectExpiry };
  }

  const productionTargetInput = correctionInput("correction:production-target");
  productionTargetInput.target.system = "storefront-production";
  const productionTarget = await postTarget("/kernel/v0/effects", command("t08-production-target",
    "kernel.effect.admit", productionTargetInput));
  assert.equal(productionTarget.response.status, 409);
  assert.equal(productionTarget.body.error.code, "EFFECT_BOUNDS_EXCEEDED");
  const wrongCredentialInput = correctionInput("correction:wrong-credential");
  wrongCredentialInput.credential_binding.revision = "storefront-writer-rev-6";
  const wrongCredential = await postTarget("/kernel/v0/effects", command("t08-wrong-credential",
    "kernel.effect.admit", wrongCredentialInput));
  assert.equal(wrongCredential.response.status, 409);
  assert.equal(wrongCredential.body.error.code, "CREDENTIAL_REVISION_MISMATCH");
  const excessiveInput = correctionInput("correction:excessive");
  excessiveInput.requested_value.quantity = 1001;
  excessiveInput.limits.maximum_quantity = 1000;
  const excessive = await postTarget("/kernel/v0/effects", command("t08-excessive",
    "kernel.effect.admit", excessiveInput));
  assert.equal(excessive.response.status, 400);
  assert.equal(excessive.body.error.code, "EFFECT_LIMIT_INVALID");

  const exactCorrection = correctionInput();
  const correctionCommand = command("t08-correction-admit", "kernel.effect.admit", exactCorrection);
  const admittedEffect = await postTarget("/kernel/v0/effects", correctionCommand);
  assert.equal(admittedEffect.response.status, 201, JSON.stringify(admittedEffect.body));
  assert.equal(admittedEffect.body.effect_record.status, "admitted");
  assert.equal(admittedEffect.body.run.execution_status, "admitted");
  assert.equal(admittedEffect.body.run.accountability_status, "pending");
  assert.equal(admittedEffect.body.execution_envelope.external_effect_authority, false);
  assert.equal(admittedEffect.body.dispatch_permit.status, "issued");
  assert.equal(admittedEffect.body.dispatch_permit.permit_document.one_use, true);
  const effectId = admittedEffect.body.effect_record.effect_id;
  const effectRunId = admittedEffect.body.effect_record.run_id;
  const permitId = admittedEffect.body.dispatch_permit.permit_id;
  const permitDigest = admittedEffect.body.dispatch_permit.permit_digest;
  const effectReplay = await postTarget("/kernel/v0/effects", correctionCommand);
  assert.equal(effectReplay.response.status, 200);
  assert.equal(effectReplay.body.effect_record.effect_id, effectId);
  const effectDomainReplay = await postTarget("/kernel/v0/effects", command("t08-correction-domain-replay",
    "kernel.effect.admit", exactCorrection));
  assert.equal(effectDomainReplay.response.status, 201);
  assert.equal(effectDomainReplay.body.domain_replayed, true);
  assert.equal(effectDomainReplay.body.effect_record.effect_id, effectId);

  const dispatchCommand = command("t08-dispatch", "kernel.effect.dispatch",
    { effect_id: effectId, permit_id: permitId, permit_digest: permitDigest });
  const dispatched = await postTarget(`/kernel/v0/effects/${effectId}/dispatch`, dispatchCommand);
  assert.equal(dispatched.response.status, 201, JSON.stringify(dispatched.body));
  assert.equal(dispatched.body.effect_record.status, "succeeded");
  assert.equal(dispatched.body.run.execution_status, "succeeded");
  assert.equal(dispatched.body.run.accountability_status, "satisfied");
  assert.ok(dispatched.body.operational_obligations.every((item) => item.status === "satisfied"));
  assert.equal(dispatched.body.evidence_record.exact_action.requested_value.quantity, 24);
  assert.equal(dispatched.body.evidence_record.adapter_result.post_write_observation.quantity, 24);
  assert.equal(dispatched.body.evidence_record.adapter_result.credential.material_returned, false);
  assert.doesNotMatch(JSON.stringify(dispatched.body), /local-storefront-scoped-credential/);
  const dispatchReplay = await postTarget(`/kernel/v0/effects/${effectId}/dispatch`, dispatchCommand);
  assert.equal(dispatchReplay.response.status, 200);
  assert.equal(dispatchReplay.body.evidence_record.evidence_record_id,
    dispatched.body.evidence_record.evidence_record_id);
  const unauthorizedDispatchReplay = await postAgent(`/kernel/v0/effects/${effectId}/dispatch`, dispatchCommand);
  assert.equal(unauthorizedDispatchReplay.response.status, 403);
  assert.equal(unauthorizedDispatchReplay.body.error.code, "EXECUTION_PASSPORT_MISMATCH");
  const consumedPermit = await kernel(`/kernel/v0/dispatch-permits/${permitId}`);
  assert.equal(consumedPermit.body.dispatch_permit.status, "consumed");
  assert.ok(consumedPermit.body.dispatch_permit.consumed_at);
  assert.ok(consumedPermit.body.dispatch_permit.brokered_at);
  const duplicateCredentialDelivery = await kernel(`/internal/v0/dispatch-permits/${permitId}/credential-delivery`, {
    method: "POST", headers: { authorization: "Bearer local-broker-service-token" },
    body: JSON.stringify({ permit_digest: permitDigest })
  });
  assert.equal(duplicateCredentialDelivery.response.status, 409);
  assert.equal(duplicateCredentialDelivery.body.error.code, "CREDENTIAL_ALREADY_DELIVERED");

  const conflictingCorrection = correctionInput();
  conflictingCorrection.requested_value.quantity = 25;
  const conflict = await postTarget("/kernel/v0/effects", command("t08-correction-conflict",
    "kernel.effect.admit", conflictingCorrection));
  assert.equal(conflict.response.status, 409);
  assert.equal(conflict.body.error.code, "EFFECT_IDEMPOTENCY_CONFLICT");
  const effectRun = await kernel(`/kernel/v0/runs/${effectRunId}`);
  assert.equal(effectRun.body.run.execution_status, "succeeded");
  assert.equal(effectRun.body.run.accountability_status, "satisfied");

  const admitUncertainEffect = async (suffix, quantity) => {
    const input = correctionInput(`storefront.inventory:set_quantity:SKU-100:${quantity}:${suffix}`);
    input.requested_value.quantity = quantity;
    const admitted = await postTarget("/kernel/v0/effects", command(`t09-${suffix}-admit`, "kernel.effect.admit", input));
    assert.equal(admitted.response.status, 201, JSON.stringify(admitted.body));
    return admitted.body;
  };

  const appliedAdmission = await admitUncertainEffect("applied", 30);
  const appliedEffectId = appliedAdmission.effect_record.effect_id;
  const appliedPermit = appliedAdmission.dispatch_permit;
  armAdapterFault("timeout_after_apply");
  const appliedDispatchInput = { effect_id: appliedEffectId, permit_id: appliedPermit.permit_id,
    permit_digest: appliedPermit.permit_digest };
  const appliedDispatchCommand = command("t09-applied-dispatch", "kernel.effect.dispatch", appliedDispatchInput);
  const appliedUncertain = await postTarget(`/kernel/v0/effects/${appliedEffectId}/dispatch`, appliedDispatchCommand);
  assert.equal(appliedUncertain.response.status, 201, JSON.stringify(appliedUncertain.body));
  assert.equal(appliedUncertain.body.effect_record.status, "uncertain");
  assert.equal(appliedUncertain.body.effect_record.was_uncertain, true);
  assert.equal(appliedUncertain.body.run.execution_status, "uncertain");
  assert.equal(appliedUncertain.body.run.accountability_status, "pending");
  const appliedCase = appliedUncertain.body.recovery_case;
  assert.equal(appliedCase.status, "open");
  assert.equal(appliedCase.reconciliation_status, "pending");
  assert.ok(appliedCase.known_facts.some((fact) => fact.fact === "target_may_have_applied_effect"));
  assert.deepEqual(appliedCase.missing_evidence, ["target_effect_receipt", "post_write_observation"]);
  assert.ok(appliedCase.deadline_at);
  assert.equal(appliedCase.responsible_actor.passport_id, targetPassportId);
  assert.deepEqual(appliedCase.allowed_options.map((option) => option.option),
    ["reconcile", "separate_corrective_work", "complete_remaining_obligations"]);
  const appliedRecoveryCaseId = appliedCase.recovery_case_id;
  const appliedReconciliationPermit = appliedCase.reconciliation_permit;

  const appliedDispatchReplay = await postTarget(`/kernel/v0/effects/${appliedEffectId}/dispatch`, appliedDispatchCommand);
  assert.equal(appliedDispatchReplay.response.status, 200);
  assert.equal(appliedDispatchReplay.body.recovery_case.recovery_case_id, appliedRecoveryCaseId);
  const blindRetry = await postTarget(`/kernel/v0/effects/${appliedEffectId}/dispatch`,
    command("t09-applied-blind-retry", "kernel.effect.dispatch", appliedDispatchInput));
  assert.equal(blindRetry.response.status, 409);
  assert.equal(blindRetry.body.error.code, "DISPATCH_PERMIT_CONSUMED");

  const openOverview = await kernel("/kernel/v0/accountable-work/overview");
  const openRecovery = openOverview.body.recovery_cases.items.find((item) => item.recovery_case_id === appliedRecoveryCaseId);
  assert.equal(openRecovery.status, "open");
  assert.equal(openRecovery.was_uncertain, true);
  const initialAppliedReconcileInput = { recovery_case_id: appliedRecoveryCaseId,
    reconciliation_permit_id: appliedReconciliationPermit.reconciliation_permit_id,
    permit_digest: appliedReconciliationPermit.permit_digest };
  armReconciliationFault();
  const failedObservation = await postTarget(`/kernel/v0/recovery-cases/${appliedRecoveryCaseId}/reconcile`,
    command("t09-applied-reconcile-failed", "kernel.recovery_case.reconcile", initialAppliedReconcileInput));
  assert.equal(failedObservation.response.status, 502);
  assert.ok(["RECONCILIATION_OBSERVATION_FAILED", "RECONCILIATION_EVIDENCE_INVALID"]
    .includes(failedObservation.body.error.code));
  const reopenedResult = await kernel(`/kernel/v0/recovery-cases/${appliedRecoveryCaseId}`);
  const reopened = reopenedResult.body.recovery_case;
  assert.equal(reopened.status, "open");
  assert.equal(reopened.reconciliation_status, "pending");
  assert.equal(reopened.reconciliation_failures.length, 1);
  assert.equal(reopened.reconciliation_permits.length, 2);
  assert.equal(reopened.reconciliation_permits[0].status, "consumed");
  assert.equal(reopened.reconciliation_permit.status, "issued");
  const appliedReconcileInput = { recovery_case_id: appliedRecoveryCaseId,
    reconciliation_permit_id: reopened.reconciliation_permit.reconciliation_permit_id,
    permit_digest: reopened.reconciliation_permit.permit_digest };
  const appliedReconcileCommand = command("t09-applied-reconcile", "kernel.recovery_case.reconcile", appliedReconcileInput);
  const appliedReconciled = await postTarget(`/kernel/v0/recovery-cases/${appliedRecoveryCaseId}/reconcile`,
    appliedReconcileCommand);
  assert.equal(appliedReconciled.response.status, 201, JSON.stringify(appliedReconciled.body));
  assert.equal(appliedReconciled.body.recovery_case.status, "resolved_applied");
  assert.equal(appliedReconciled.body.recovery_case.reconciliation_status, "applied");
  assert.equal(appliedReconciled.body.recovery_case.was_uncertain, true);
  assert.equal(appliedReconciled.body.effect_record.status, "succeeded");
  assert.equal(appliedReconciled.body.effect_record.was_uncertain, true);
  assert.equal(appliedReconciled.body.run.execution_status, "succeeded");
  assert.equal(appliedReconciled.body.run.accountability_status, "satisfied");
  assert.equal(appliedReconciled.body.evidence_record.evidence_type, "reconciled_effect_verification");
  assert.equal(appliedReconciled.body.evidence_record.effect_receipt.found, true);
  assert.equal(appliedReconciled.body.evidence_record.effect_receipt.idempotency_key,
    "storefront.inventory:set_quantity:SKU-100:30:applied");
  assert.equal(appliedReconciled.body.evidence_record.observation.quantity, 30);
  assert.ok(appliedReconciled.body.evidence_record.obligation_evidence.every((entry) => entry.satisfied));
  assert.equal(appliedReconciled.body.evidence_record.preserves_uncertainty_history, true);
  assert.ok(appliedReconciled.body.recovery_case.reconciliations[0].immutable_record);
  const appliedReconcileReplay = await postTarget(`/kernel/v0/recovery-cases/${appliedRecoveryCaseId}/reconcile`,
    appliedReconcileCommand);
  assert.equal(appliedReconcileReplay.response.status, 200);
  const duplicateReconcile = await postTarget(`/kernel/v0/recovery-cases/${appliedRecoveryCaseId}/reconcile`,
    command("t09-applied-reconcile-again", "kernel.recovery_case.reconcile", appliedReconcileInput));
  assert.equal(duplicateReconcile.response.status, 409);
  assert.equal(duplicateReconcile.body.error.code, "RECONCILIATION_NOT_AVAILABLE");

  const notAppliedAdmission = await admitUncertainEffect("not-applied", 31);
  const notAppliedEffectId = notAppliedAdmission.effect_record.effect_id;
  const notAppliedPermit = notAppliedAdmission.dispatch_permit;
  armAdapterFault("drop_before_apply");
  const notAppliedDispatchInput = { effect_id: notAppliedEffectId, permit_id: notAppliedPermit.permit_id,
    permit_digest: notAppliedPermit.permit_digest };
  const notAppliedUncertain = await postTarget(`/kernel/v0/effects/${notAppliedEffectId}/dispatch`,
    command("t09-not-applied-dispatch", "kernel.effect.dispatch", notAppliedDispatchInput));
  assert.equal(notAppliedUncertain.response.status, 201, JSON.stringify(notAppliedUncertain.body));
  assert.equal(notAppliedUncertain.body.effect_record.status, "uncertain");
  const notAppliedCase = notAppliedUncertain.body.recovery_case;
  const notAppliedReconcileInput = { recovery_case_id: notAppliedCase.recovery_case_id,
    reconciliation_permit_id: notAppliedCase.reconciliation_permit.reconciliation_permit_id,
    permit_digest: notAppliedCase.reconciliation_permit.permit_digest };
  const notAppliedReconciled = await postTarget(`/kernel/v0/recovery-cases/${notAppliedCase.recovery_case_id}/reconcile`,
    command("t09-not-applied-reconcile", "kernel.recovery_case.reconcile", notAppliedReconcileInput));
  assert.equal(notAppliedReconciled.response.status, 201, JSON.stringify(notAppliedReconciled.body));
  assert.equal(notAppliedReconciled.body.recovery_case.status, "open_not_applied");
  assert.equal(notAppliedReconciled.body.recovery_case.reconciliation_status, "not_applied");
  assert.equal(notAppliedReconciled.body.recovery_case.was_uncertain, true);
  assert.equal(notAppliedReconciled.body.effect_record.status, "failed");
  assert.equal(notAppliedReconciled.body.effect_record.was_uncertain, true);
  assert.equal(notAppliedReconciled.body.run.execution_status, "failed");
  assert.equal(notAppliedReconciled.body.run.accountability_status, "breached");
  assert.ok(notAppliedReconciled.body.run.obligations.every((obligation) => obligation.status === "breached"));
  assert.ok(notAppliedReconciled.body.run.obligations.every((obligation) => obligation.breached_at));
  assert.ok(notAppliedReconciled.body.run.obligations.every((obligation) =>
    obligation.resolution_detail.reason === "original_effect_not_applied"));
  assert.equal(notAppliedReconciled.body.recovery_case.reconciliations[0].observation.effect_receipt.found, false);
  assert.equal(notAppliedReconciled.body.recovery_case.reconciliations[0].observation.target_observation.quantity, 30);
  assert.equal(notAppliedReconciled.body.evidence_record, null);
  const correctiveDraft = notAppliedReconciled.body.corrective_work_intent_proposal;
  assert.equal(correctiveDraft.status, "requires_normal_submission");
  assert.equal(correctiveDraft.operation_id, "kernel.work_intent.propose");
  assert.equal(correctiveDraft.input.constraints.no_retry_of_original_effect, true);
  assert.deepEqual(correctiveDraft.input.constraints.required_authority_sequence,
    ["delegation", "capability", "execution_envelope", "run", "effect", "evidence"]);
  const correctiveProposal = await postTarget("/kernel/v0/work-intent-proposals",
    command("t09-corrective-work-proposal", correctiveDraft.operation_id, correctiveDraft.input));
  assert.equal(correctiveProposal.response.status, 201, JSON.stringify(correctiveProposal.body));
  assert.equal(correctiveProposal.body.proposal.status, "proposed");
  assert.equal(Object.hasOwn(correctiveProposal.body.proposal, "confirmed_work_intent_id"), false);

  const recoveredOverview = await kernel("/kernel/v0/accountable-work/overview");
  const projectedApplied = recoveredOverview.body.effects.items.find((item) => item.effect_id === appliedEffectId);
  const projectedNotApplied = recoveredOverview.body.effects.items.find((item) => item.effect_id === notAppliedEffectId);
  assert.equal(projectedApplied.effect_status, "succeeded");
  assert.equal(projectedApplied.was_uncertain, true);
  assert.equal(projectedApplied.recovery_case.status, "resolved_applied");
  assert.equal(projectedNotApplied.effect_status, "failed");
  assert.equal(projectedNotApplied.was_uncertain, true);
  assert.equal(projectedNotApplied.recovery_case.status, "open_not_applied");
  ...
  await explicitWait(Math.max(0, Date.parse(acceptedDelegationExpiry) - Date.now() + 1_000));
  const expiredDelegationInput = admissionInput("comparison:expired-delegation");
  expiredDelegationInput.expires_at = new Date(Date.now() + 60_000).toISOString();
  const expiredDelegation = await postTarget("/kernel/v0/execution-envelopes", command("t07-expired-delegation",
    "kernel.execution_envelope.admit", expiredDelegationInput));
  assert.equal(expiredDelegation.response.status, 409);
  assert.equal(expiredDelegation.body.error.code, "DELEGATION_EXPIRED");
  ...
  assert.equal(handoffOverview.body.runs.count, 4);
  const projectedRun = handoffOverview.body.runs.items.find((item) => item.run_id === runId);
  assert.equal(projectedRun.execution.status, "completed");
  assert.equal(projectedRun.accountability.status, "satisfied");
  assert.equal(projectedRun.evidence.evidence_record_id, evidenceRecordId);
  assert.ok(projectedRun.evidence.source_links.every((link) => link.context_receipt_id === runtimeContextReceiptId));
  assert.equal(handoffOverview.body.effects.count, 3);
  const projectedEffect = handoffOverview.body.effects.items.find((item) => item.effect_id === effectId);
  assert.equal(projectedEffect.effect_id, effectId);
  assert.equal(projectedEffect.execution_status, "succeeded");
  assert.equal(projectedEffect.accountability_status, "satisfied");
  assert.equal(projectedEffect.target.subject, "SKU-100");
  assert.equal(projectedEffect.requested_value.quantity, 24);
  assert.equal(projectedEffect.evidence.outcome, "succeeded");
  assert.doesNotMatch(JSON.stringify(handoffOverview.body), /local-storefront-scoped-credential/);
  const butlerResponse = await fetch(`${kernelUrl}/butler`, { headers: { authorization: "Bearer local-development-bootstrap-token" } });
  const butlerHtml = await butlerResponse.text();
  assert.equal(butlerResponse.status, 200);
  for (const value of [effectId, appliedRecoveryCaseId, notAppliedCase.recovery_case_id, capabilityActivationId,
    runtimeContextReceiptId, permitId, "set_quantity", "SKU-100", "succeeded", "satisfied", "storefront_response",
    "restore_previous_quantity", "resolved_applied", "open_not_applied", "was_uncertain"]) {
    assert.match(butlerHtml, new RegExp(value));
  }
  ...
  const persistedRun = await kernel(`/kernel/v0/runs/${runId}`);
  assert.equal(persistedRun.body.run.execution_status, "completed");
  assert.equal(persistedRun.body.run.accountability_status, "satisfied");
  const persistedEvidence = await kernel(`/kernel/v0/evidence-records/${evidenceRecordId}`);
  assert.equal(persistedEvidence.body.evidence_record.evidence_digest, completed.body.evidence_record.evidence_digest);
  const persistedEffect = await kernel(`/kernel/v0/effects/${effectId}`);
  assert.equal(persistedEffect.body.effect_record.status, "succeeded");
  const persistedPermit = await kernel(`/kernel/v0/dispatch-permits/${permitId}`);
  assert.equal(persistedPermit.body.dispatch_permit.status, "consumed");
  const persistedEffectRun = await kernel(`/kernel/v0/runs/${effectRunId}`);
  assert.equal(persistedEffectRun.body.run.execution_status, "succeeded");
  assert.equal(persistedEffectRun.body.run.accountability_status, "satisfied");

  const persistedAppliedCase = await kernel(`/kernel/v0/recovery-cases/${appliedRecoveryCaseId}`);
  assert.equal(persistedAppliedCase.body.recovery_case.status, "resolved_applied");
  assert.equal(persistedAppliedCase.body.recovery_case.was_uncertain, true);
  const persistedNotAppliedCase = await kernel(`/kernel/v0/recovery-cases/${notAppliedCase.recovery_case_id}`);
  assert.equal(persistedNotAppliedCase.body.recovery_case.status, "open_not_applied");

  assert.notEqual(passportId, targetPassportId);
  const finishedAt = Date.now();
  const observable = {
    schema_version: "alphonse.engineering_rehearsal_observation.v0.1",
    workflow: {
      plain_language_input: plainLanguageWorkflow,
      initial_operational_packages: initialOverview.body.accountable_work.count,
      package: "com.alphonse.inventory.correction@0.1.0"
    },
    runtime_clients: { count: 2, distinct_passports: true, handoff_without_ambient_memory: true },
    journey: {
      intent: "confirmed",
      context: "receipt_recorded",
      package: "published",
      validation: "passed",
      simulation: ["deterministic_fixture_passed", "observational_read_only_passed"],
      deployment: "staged",
      activation: "active",
      handoff: "accepted",
      comparison: { discrepancy: 6, correction_required: true },
      correction: { target: "storefront-staging", subject: "SKU-100", quantity: 24, status: "succeeded" },
      evidence: "satisfied",
      uncertainty: ["timeout_after_apply", "drop_before_apply"],
      recovery: ["resolved_applied", "open_not_applied_with_corrective_intent"]
    },
    negative_matrix: {
      secrets: "SECRET_MATERIAL_PROHIBITED",
      stale_context: "STALE_CONTEXT",
      wrong_identity: "EXECUTION_PASSPORT_MISMATCH",
      inactive_authority: "CAPABILITY_INACTIVE",
      duplicate_admission: "domain_replayed",
      duplicate_dispatch: "original_evidence_returned",
      expired_lease: "expired",
      expired_delegation: "DELEGATION_EXPIRED",
      missing_evidence: "EVIDENCE_REQUIRED"
    },
    butler_projection: {
      deployments: handoffOverview.body.deployments.count,
      handoff_states: [...handoffStates].sort(),
      runs: handoffOverview.body.runs.count,
      effects: handoffOverview.body.effects.count,
      applied_recovery: projectedApplied.recovery_case.status,
      not_applied_recovery: projectedNotApplied.recovery_case.status
    },
    persistence: {
      activation: persistedActivation.body.capability_activation.state,
      handoff: persistedHandoff.body.handoff.state,
      comparison_run: `${persistedRun.body.run.execution_status}/${persistedRun.body.run.accountability_status}`,
      correction_effect: persistedEffect.body.effect_record.status,
      applied_recovery: persistedAppliedCase.body.recovery_case.status,
      not_applied_recovery: persistedNotAppliedCase.body.recovery_case.status
    }
  };
  const timing = {
    elapsed_ms: finishedAt - rehearsalMetrics.started_at_ms,
    environment_setup_ms: rehearsalMetrics.environment_setup_ms,
    explicit_wait_ms: rehearsalMetrics.explicit_wait_ms,
    active_automation_ms: Math.max(0, finishedAt - rehearsalMetrics.started_at_ms
      - rehearsalMetrics.environment_setup_ms - rehearsalMetrics.explicit_wait_ms),
    measured_human_attention_ms: 0,
    simulated_human_decisions: 4
  };

  if (process.env.EMIT_REHEARSAL_RESULT === "1") {
    console.log(`ALPHONSE_REHEARSAL_RESULT=${JSON.stringify({ observable, timing })}`);
  }

  console.log("Ticket 09 black-box acceptance passed.");
} finally {
  if (process.env.KEEP_TICKET09_STACK !== "1") compose("down", "--volumes", "--remove-orphans");
}
