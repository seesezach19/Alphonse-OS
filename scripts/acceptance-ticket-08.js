...
const composeEnvironment = {
  ...process.env,
  COMPOSE_PROJECT_NAME: "alphonse-kernel-ticket-08-acceptance",
  KERNEL_PORT: "43106",
  POSTGRES_PORT: "45437",
...
async function waitUntilHealthy() {
  ...
  throw new Error("Ticket 08 services did not become healthy.");
}
...
try {
  ...
  await new Promise((resolve) => setTimeout(resolve, Math.max(0, Date.parse(acceptedDelegationExpiry) - Date.now() + 1_000)));
  ...
  assert.equal(handoffOverview.body.runs.count, 2);
  ...
  assert.equal(handoffOverview.body.effects.count, 1);
  const projectedEffect = handoffOverview.body.effects.items[0];
  ...
  for (const value of [effectId, capabilityActivationId, runtimeContextReceiptId, permitId,
    "set_quantity", "SKU-100", "succeeded", "satisfied", "storefront_response", "restore_previous_quantity"]) {
  ...
  console.log("Ticket 08 black-box acceptance passed.");
} finally {
  if (process.env.KEEP_TICKET08_STACK !== "1") compose("down", "--volumes", "--remove-orphans");
}
