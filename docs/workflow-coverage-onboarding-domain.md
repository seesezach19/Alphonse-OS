...
| Runtime adapter | Contract 0.4.0 publishes required `runtime_adapter.execution_history.list` and `runtime_adapter.workflow_inventory.list` alongside workflow identity, revision, event, detail, replay, and health seams (`src/workflow-runtime-adapter-contract.js`). Its closed schemas grant no authority. | Inventory remains discovery-only. Execution history supplies credential-scoped cursor pages with a fixed cutoff, explicit production/manual/test/retry classes, and exact revision evidence; provider credentials remain at the adapter edge. |
...
| n8n observation and reconciliation | The canonical observer persists a provider cursor and fixed source cutoff and walks the credential-scoped n8n public execution API. Embedded workflow signals only accelerate individual reads and are explicitly never completeness proof (`src/canonical-n8n-observer.js`). The first-party adapter emits normalized authority-free pages with signed scope/workflow/cutoff/page-bound cursors and revision fingerprints (`packages/n8n-operational-package/src/execution-history.js`). Diagnostic admits each exact page and execution into an append-only ledger, fails closed on provider outage or malformed pages, and projects gaps plus immutable closed intervals (`src/coverage-reconciliation-service.js`, `diagnostic-migrations/029_coverage_reconciliation_intervals.sql`). | A complete cursor walk is the only basis for `active`. Provider retention/deletion remains a disclosed limitation; missing prior executions, unknown modes, incomplete production executions, unavailable revision evidence, and revision drift remain visible. Drift suspends revision-dependent coverage. Recovery requires another completed walk and never rewrites the prior interval. |
...
| Coverage interval | **Implemented immutable derived projection over append-only reconciliation evidence.** | Each completed walk or durable degradation closes the prior half-open interval. Historical gaps and limitations remain bound by digest; the open current interval changes only after a new admitted transition. |
...
Coverage Onboarding and Coverage Review Approval remain the durable domain records. Reconciliation events, pages,
and normalized execution observations are append-only Diagnostic evidence records; intervals are verified projections,
not mutable truth rows or new authority-bearing aggregates.

## Execution-History Reconciliation Boundary

`diagnostic.coverage_reconciliation.advance` performs exactly one bounded adapter read under the authenticated
onboarding Passport. The caller supplies only the expected reconciliation revision, active cycle identity, and page
size. It cannot supply a page, cursor, assessment, interval, gap, or desired state. The service reads the durable
cursor, validates an exact adapter page, recomputes its page digest, stores the page artifact and normalized execution
observations, and appends digest-chained events. A repeated command ID with identical bytes replays without another
provider read; changed bytes conflict before provider access.

The first page fixes `source_cutoff`; every continuation cursor is signed by the first-party adapter and binds the
credential scope digest, provider workflow ID, cutoff, provider cursor, and page index. A completed walk compares its
execution identities to the preceding completed walk. Provider disappearance is a blocking gap, later reappearance is
a visible late-fill limitation, and manual/test executions remain present but excluded from production coverage.
Provider unavailability appends a durable suspension event while retaining the active resume cursor. Revision drift
also suspends. Neither condition can be cleared by a workflow signal or caller assertion; only a later complete,
drift-free cursor walk opens a new active interval.

Every completed or degraded event identifies the exact reconciler stage artifact. That identity closes over the
Kernel contracts and service, immutable migration, canonical observer, runtime-adapter contract, first-party n8n
history and attestation code, package manifests, runtime, and dependency lockfile. All outputs grant `authority: none`
and make no destination-commitment, registration, execution, repair, verification, promotion, or business claim.
