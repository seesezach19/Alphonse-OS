...
function recordCardinality(values, missing, effectId, role, missingReason, context = {}) {
  ...
  missing.push({ effect_id: effectId, role,
    ...
    candidate_count: values.length, ...context });
}

function evidenceReference(node, bases) {
  return { receipt_id: node.receipt_reference.receipt_id,
    receipt_digest: node.receipt_reference.receipt_digest,
    intake_position: String(node.receipt_reference.intake_position),
    observation_type: node.node_type, node_key: node.node_key,
    selection_bases: [...bases].sort() };
}

export function selectDiagnosticEvidence({ correlationProjection, effectProjection, behaviorEvaluation,
  observationEvidence, selectionPolicy }) {
  ...
  const selectEdge = (edge) => { if (edge) selectedEdgeKeys.add(edge.edge_key); };
  ...
  for (const matched of behaviorEvaluation.measurement.matched_effects) {
    ...
    if (!effect || sha256Digest(effect) !== matched.effect_digest || effect.status !== "committed") {
      fail("VERIFIER_MATCHED_EFFECT_INTEGRITY_VIOLATION", { effect_id: matched.effect_id });
    }
      ...
      for (const requestEdge of requestEdges) {
        ...
        for (const deliveryEdge of deliveryEdges) {
          ...
          for (const operationEdge of operationEdges) {
            ...
            } else missingRoles.push({ effect_id: effect.effect_id, role: "source_delivery",
              reason: "logical_operation_node_missing", candidate_count: 0,
              from_node_key: deliveryNode.node_key, candidate_node_key: operationEdge.from_node_key });
          }
          ...
          recordCardinality(runtimeEdges, missingRoles, effect.effect_id,
            "terminal_runtime_execution", "typed_runtime_path_missing", { from_node_key: deliveryNode.node_key });
          for (const runtimeEdge of runtimeEdges) {
            ...
            if (!runtimeNode || runtimeNode.node_type !== "runtime.execution" || !runtimeEvidence
                || !["cancelled", "failed", "succeeded"].includes(runtimeEvidence.envelope.claims?.lifecycle)) {
              missingRoles.push({ effect_id: effect.effect_id, role: "terminal_runtime_execution",
                ...
                from_node_key: deliveryNode.node_key, candidate_node_key: runtimeEdge.to_node_key });
            } else {
  ...
  const allowedRelationships = new Set(selectionPolicy.allowed_relationships);
  for (const edge of edges) {
    if (allowedRelationships.has(edge.relationship) && selectedNodeKeys.has(edge.from_node_key)
        && selectedNodeKeys.has(edge.to_node_key)) selectEdge(edge);
  }
  ...
  const selectedObservations = selectedNodes.filter((node) => node.receipt_reference).map((node) => {
    ...
    if (!evidence || evidence.receipt_digest !== node.receipt_reference.receipt_digest) {
      fail("VERIFIER_SELECTED_OBSERVATION_INTEGRITY_VIOLATION", { node_key: node.node_key });
    }
    return { ...evidenceReference(node, selectionBasesByNode.get(node.node_key)),
  ...
  for (const reference of selectedEdges.flatMap((edge) => edge.supporting_tokenization_provenance ?? [])) {
  ...
  for (const reference of [...provenanceReferences.values()].sort(compareCanonical)) {
    ...
    if (!material || (reference.receipt_digest && material.receipt_digest !== reference.receipt_digest)
        || (reference.grant_snapshot_digest && material.grant_snapshot_digest !== reference.grant_snapshot_digest)
  ...
  const omittedDetails = correlationProjection.manifests.receipts.filter((receipt) =>
    ...
    reason: selectionPolicy.detail.omission_reason })).sort(compareCanonical);
  ...
  return {
    ...
    role_completion: { required_roles: [...selectionPolicy.required_roles].sort(),
    ...
    disclosure_accounting: { selection_seed: "matched_committed_effects",
      ...
      excluded_related_counts_by_type: excludedRelatedCounts, omitted_detail: omittedDetails,
